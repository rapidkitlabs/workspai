...
export async function createFrontendProject(
  ...
  if (exitCode !== 0 && scaffoldReady) {
    console.log(
      chalk.yellow(
        `⚠️  Official ${definition.displayName} generator exited with code ${exitCode}, but the scaffold looks complete. Continuing RapidKit project setup...`
      )
    );
  }
...
async function writeFrontendRapidkitMetadata(input: {
  ...
  await fsExtra.ensureDir(path.join(input.projectPath, '.rapidkit'));
  await fsExtra.writeJson(path.join(input.projectPath, '.rapidkit', 'project.json'), projectJson, {
    spaces: 2,
  });
  await fsExtra.writeJson(path.join(input.projectPath, '.rapidkit', 'context.json'), contextJson, {
    spaces: 2,
  });
  await fsExtra.writeJson(
    path.join(input.projectPath, '.rapidkit', 'frontend-create.json'),
    evidenceJson,
    { spaces: 2 }
  );
}
...
function printFrontendPlan(input: {
  ...
  console.log(chalk.bold(`\nRapidKit frontend create plan: ${input.definition.displayName}`));
