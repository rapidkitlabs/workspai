...
export async function cleanupAdoptedProjectImport(
  ...
  const projectJsonPath = path.join(projectPath, '.rapidkit', 'project.json');
  const adoptJsonPath = path.join(projectPath, '.rapidkit', 'adopt.json');
  const adoptReadinessPath = path.join(projectPath, '.rapidkit', 'adopt-readiness.json');

  if (previousProjectJson) {
    await fsExtra.writeJson(projectJsonPath, previousProjectJson, { spaces: 2 });
  } else if (await fsExtra.pathExists(projectJsonPath)) {
