...
function normalizeBootstrapCommand(command?: string): string {
  const base = command?.trim() || 'npx rapidkit bootstrap';
  ...
  return `${JSON.stringify(
    {
      ...
      generatedAt,
      source: 'rapidkit workspace remediation-plan',
      runtimes: {},
      notes: [
...
function bootstrapActions(input: {
  ...
  const verifyCommand = normalizeBootstrapCommand('npx rapidkit bootstrap --json');
    ...
    if (blocker.includes('profile.enterprise.compatibility-matrix')) {
      actions.push(
        actionBase({
          ...
          risk: 'safe',
          verifyCommand,
          files: ['.rapidkit/compatibility-matrix.json'],
          operation: {
            type: 'file-create',
            path: '.rapidkit/compatibility-matrix.json',
    ...
    if (blocker.includes('profile.enterprise.mirror-config')) {
      actions.push(
        actionBase({
          ...
          risk: 'safe',
          verifyCommand,
          files: ['.rapidkit/mirror-config.json'],
          operation: {
            type: 'file-create',
            path: '.rapidkit/mirror-config.json',
  ...
  const byKind: Record<string, { title: string; phase: string; command: string; verify: string }> =
    {
      analyze: {
        ...
        phase: 'analysis',
        command: 'npx rapidkit analyze --strict --json',
        verify: 'npx rapidkit analyze --strict --json',
      },
      readiness: {
      ...
      pipeline: {
        ...
        verify: 'npx rapidkit pipeline --json --strict',
      },
      'workspace-run': {
        ...
        command: input.ciMode
          ...
          : 'npx rapidkit workspace run test --json',
        verify: input.ciMode
          ? 'npx rapidkit workspace run test --strict --json'
          : 'npx rapidkit workspace run test --json',
      },
      'workspace-verify': {
        ...
        phase: 'verification',
        command: input.ciMode
          ? 'npx rapidkit workspace verify --strict --json'
      ...
      'doctor-workspace': {
        ...
        command: 'npx rapidkit doctor workspace --plan --json',
        verify: 'npx rapidkit doctor workspace --json',
      },
    };
...
export async function writeArtifactRemediationPlan(
  ...
  await fsExtra.writeJSON(outputPath, plan, { spaces: 2 });
  return outputPath;
}
