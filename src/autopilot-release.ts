...
function buildNextActions(report: {
  ...
  if (report.executionError) {
    return [
      'Re-run: npx rapidkit autopilot release --mode audit --json',
      'Inspect .rapidkit/reports/autopilot-release-last-run.json for execution failure details',
    ];
  }

  if (report.blockers.length > 0) {
    return [
      'Run: npx rapidkit doctor workspace --plan',
      'Run: npx rapidkit readiness --json --strict',
      'Run: npx rapidkit workspace run test --affected --strict',
    ];
  }
  ...
  if (report.hasWarnings && report.mode !== 'enforce') {
    return [
      ...
      'Optionally run: npx rapidkit autopilot release --mode safe-fix',
    ];
  }
