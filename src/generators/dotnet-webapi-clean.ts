...
{
    public const string SectionName = "RapidKit";
...
export async function generateDotnetWebApiCleanKit(
  ...
  const rootNamespace = sanitizeNamespace(
    variables.root_namespace || `RapidKit.${toPascalCase(projectName)}`,
    'RapidKit.App'
  );
  ...
  const v: Required<DotnetWebApiCleanVariables> = {
    ...
    description: sanitizeHumanText(
      ...
      'Production-ready ASP.NET Core Web API scaffolded by RapidKit.'
    ),
  ...
  await writeGeneratorFile(path.join(projectPath, '.rapidkit', 'context.json'), contextJson());
  await writeGeneratorFile(
  ...
  console.log(chalk.gray('ℹ  RapidKit modules are not available for .NET projects yet.'));
}
