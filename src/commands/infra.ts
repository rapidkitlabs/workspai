...
async function ensurePlanExists(workspacePath: string): Promise<InfraPlan> {
  ...
  if (!(await fsExtra.pathExists(planPath))) {
    throw new Error(
      `Infra plan not found at ${INFRA_PLAN_RELATIVE_PATH}. Run: npx rapidkit infra plan`
    );
  }
...
export async function runDockerCompose(input: {
  ...
  if (!(await fsExtra.pathExists(composePath))) {
    throw new Error(
      `Compose file not found at ${INFRA_COMPOSE_RELATIVE_PATH}. Run: npx rapidkit infra plan`
    );
  }
    ...
    .action(
      async (options: {
        ...
        try {
          ...
          if (plan.services.length === 0) {
            throw new Error(
              'Infra plan has no services. Run: npx rapidkit infra plan (from a workspace with .env.example, core modules, or .rapidkit/infra/overrides.json)'
            );
          }
          ...
          if (options.detach !== false) {
            ...
            console.log(chalk.gray('Check status: npx rapidkit infra status\n'));
          }
        } catch (error) {
          console.error(
            chalk.red(`\n❌ ${error instanceof Error ? error.message : String(error)}\n`)
    ...
    .action(async (options: { workspace?: string; json?: boolean; strict?: boolean }) => {
      try {
        ...
        if (result.stdout.trim()) {
          ...
          console.log(chalk.gray('Start with: npx rapidkit infra up\n'));
        }
        ...
        if (options.strict && /Restarting|Exit|unhealthy/i.test(result.stdout)) {
          ...
          console.log(
            chalk.gray('Inspect logs with: docker logs rapidkit-postgres (or redis/mailpit)\n')
          );
          process.exit(1);
        }
      } catch (error) {
