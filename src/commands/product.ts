...
export async function buildPrivateProductManifest(params: {
  ...
  const outputRoot = params.outputRoot || '../rapidkit-examples-pro';
  const manifestWithoutChecksum: PrivateProductManifest = {
    ...
    factory: {
      ...
      manifestChecksum: '',
      requiredCommands: [
        `npx rapidkit create workspace ${product.slug} --yes --profile enterprise`,
        `npx rapidkit create project ${product.recommendedKit} api --yes --skip-install`,
        'npx rapidkit init',
        'npx rapidkit workspace run test --strict --json',
        'npx rapidkit readiness --strict --json',
      ],
      releaseEvidencePath: `.rapidkit/product-factory/${product.slug}/release-evidence.json`,
    },
  };
