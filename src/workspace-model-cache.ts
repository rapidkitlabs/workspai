...
export const WORKSPACE_MODEL_CACHE_PATH = '.rapidkit/cache/workspace-model.v1.json';
...
export async function readWorkspaceModelCache(
  ...
  const cachePath = path.join(path.resolve(workspacePath), WORKSPACE_MODEL_CACHE_PATH);
  try {
...
export async function writeWorkspaceModelCache(
  ...
  await fsExtra.writeJson(cachePath, full, { spaces: 2 });
  return cachePath;
}
