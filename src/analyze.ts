...
async function analyzeProject(workspacePath: string, projectPath: string): Promise<AnalyzeProject> {
  const projectJson = await readJsonObject(path.join(projectPath, '.rapidkit', 'project.json'));
  ...
  const hasRapidKitMarker =
    (await pathExists(path.join(projectPath, '.rapidkit', 'project.json'))) ||
    (await pathExists(path.join(projectPath, '.rapidkit', 'context.json')));
  ...
  if (detection.key === 'unknown') {
    findings.push(
      finding(
        ...
        'RapidKit cannot confidently classify this backend project.',
        'Add .rapidkit/project.json metadata or import the project with `rapidkit import`.'
      )
    );
  }
  if (!hasRapidKitMarker) {
    findings.push(
      finding(
        ...
        'warn',
        target,
        'RapidKit marker is missing',
        'The project can be detected by files, but it is not registered with RapidKit metadata.',
        'Run `rapidkit import <path>` from a workspace or create the project through RapidKit.'
      )
    );
  }
  if (!hasTests) {
    findings.push(
      finding(
        ...
        'Add a test command so `rapidkit workspace run test --affected` can gate changes.'
      )
    );
  }
  if (!hasEnvExample) {
...
async function readWorkspaceProfile(workspacePath: string): Promise<string | null> {
  const workspaceJson = await readJsonObject(
...
function buildNextActions(report: {
  ...
  if (report.findings.some((item) => item.id === 'workspace.marker.missing')) {
    actions.push('Initialize workspace metadata with `rapidkit bootstrap --profile polyglot`.');
  }
  if (report.findings.some((item) => item.id === 'project.marker.missing')) {
    actions.push(
      'Register detected projects with `rapidkit import <path>` or recreate them via `rapidkit create project`.'
    );
  }
  if (report.findings.some((item) => item.id === 'project.tests.missing')) {
    actions.push(
      'Add test entrypoints, then gate changes with `rapidkit workspace run test --affected --strict`.'
    );
  }
  ...
  if (!report.hasGraph) {
    actions.push(
      'Create `.rapidkit/workspace-dependency-graph.json` or use analyze output as the first graph seed.'
    );
  }
  actions.push('Run `rapidkit autopilot release --mode audit --json` before release.');
...
export async function runAnalyze(options: AnalyzeOptions = {}): Promise<AnalyzeReport> {
  ...
  const workspaceDetected =
    (await pathExists(path.join(workspacePath, '.rapidkit-workspace'))) ||
    (await pathExists(path.join(workspacePath, '.rapidkit', 'workspace.json')));
  ...
  if (!workspaceDetected) {
    workspaceFindings.push(
      finding(
        ...
        'The directory can be analyzed, but it is not a registered RapidKit workspace.',
        'Run `rapidkit create workspace` or `rapidkit bootstrap --profile polyglot` in a workspace root.'
      )
    );
  }
  if (projects.length === 0) {
    workspaceFindings.push(
      finding(
        ...
        'Create a project with `rapidkit create project` or import one with `rapidkit import <path>`.'
      )
    );
  }
  ...
  const report: AnalyzeReport = {
    ...
    enterpriseControls: {
      jsonReady: true,
      ciGateCommand: 'rapidkit analyze --json --strict',
      releaseGateCommand: 'rapidkit autopilot release --mode enforce --json',
