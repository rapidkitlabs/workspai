...
export function buildWorkspaceProfileBootstrapCommand(
  ...
  return `npx rapidkit bootstrap --profile ${profile}${options.ci ? ' --ci --json' : ''}`;
}
...
export async function readWorkspaceProfilePolicyMode(
  ...
  const policyFilePath = path.join(workspacePath, '.rapidkit', 'policies.yml');
  if (!(await fsExtra.pathExists(policyFilePath))) {
    return 'warn';
  }
...
export async function readWorkspaceManifestProfile(
  ...
  if (!(await fsExtra.pathExists(workspaceJsonPath))) {
    return 'minimal';
  }
