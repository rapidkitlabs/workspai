...
export async function runPipeline(options: PipelineOptions = {}): Promise<PipelineReport> {
  ...
  try {
    ...
    if (!sync.workspaceFound) {
      blockingReasons.push(
        'workspace registry entry missing — run rapidkit create workspace or register manually'
      );
    }
  } catch (error) {
    executionError = true;
  ...
  try {
    ...
    const doctorEvidence = path.join(workspacePath, '.rapidkit', 'reports', 'doctor-last-run.json');
    stages.push({
      name: 'doctor',
      status: doctorStatus,
