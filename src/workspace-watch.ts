...
/**
 * Generated `.rapidkit` output subdirectories. These must be ignored (the model
 ...
 */
const RAPIDKIT_GENERATED_SUBDIRS = new Set(['reports', 'cache', 'tmp', '.cache']);
...
export function isWatchRelevantPath(relativePath: string): boolean {
  ...
  for (let i = 0; i < segments.length - 1; i += 1) {
    if (segments[i] === '.rapidkit' && RAPIDKIT_GENERATED_SUBDIRS.has(segments[i + 1])) {
      return false;
    }
  }
  return true;
