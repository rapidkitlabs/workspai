...
async function makeWorkspace(): Promise<string> {
  ...
  await fsExtra.writeFile(path.join(workspacePath, '.rapidkit-workspace'), '1\n');
...
describe('artifact remediation plan', () => {
  it('builds deterministic Bootstrap compliance remediation actions for Studio', async () => {
    ...
    await fsExtra.writeJSON(
      path.join(workspacePath, '.rapidkit', 'reports', 'bootstrap-compliance.latest.json'),
      {
    ...
    expect(plan.actions[0].command).toBe('npx rapidkit bootstrap --ci --json');
  ...
  it('builds CI-oriented verify commands when requested', async () => {
    ...
    await fsExtra.writeJSON(
      path.join(workspacePath, '.rapidkit', 'reports', 'workspace-run-last.json'),
      {
    ...
    expect(plan.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          cardId: 'readiness',
          command: 'npx rapidkit readiness --strict --json',
          verifyCommand: 'npx rapidkit readiness --strict --json',
        }),
        expect.objectContaining({
          cardId: 'workspaceRun',
          command: 'npx rapidkit workspace run test --strict --json',
          verifyCommand: 'npx rapidkit workspace run test --strict --json',
        }),
      ])
    );
  });
