...
describe('workspace Python engine install gate', () => {
  ...
  it('keeps module support tied to RapidKit module-enabled kit metadata', async () => {
    ...
    await fsExtra.ensureDir(path.join(customProjectPath, '.rapidkit'));
    await fsExtra.outputJson(path.join(customProjectPath, '.rapidkit', 'project.json'), {
      runtime: 'python',
      framework: 'fastapi',
      kit_name: 'adopted.fastapi',
    });
    ...
    await fsExtra.ensureDir(path.join(kitProjectPath, '.rapidkit'));
    await fsExtra.outputJson(path.join(kitProjectPath, '.rapidkit', 'project.json'), {
      runtime: 'python',
      framework: 'fastapi',
  ...
  itOnPosix(
    ...
    async () => {
      ...
      await fsExtra.ensureDir(path.join(moduleProjectPath, '.rapidkit'));
      await fsExtra.outputJson(path.join(moduleProjectPath, '.rapidkit', 'project.json'), {
        runtime: 'python',
        framework: 'fastapi',
        kit_name: 'fastapi.standard',
      });
      ...
      const marker = await fsExtra.readJson(path.join(workspacePath, '.rapidkit-workspace'));
