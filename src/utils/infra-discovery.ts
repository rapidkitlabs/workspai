...
async function readInfraOverrides(workspacePath: string): Promise<string[]> {
  const overridePath = path.join(workspacePath, '.rapidkit', 'infra', 'overrides.json');
  if (!(await fsExtra.pathExists(overridePath))) {
    return [];
  }
...
async function readContractEnvVars(
  ...
  if (!(await fsExtra.pathExists(contractPath))) {
    return [];
  }
