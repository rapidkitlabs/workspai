...
async function hasFileWithSuffix(root: string, suffix: string, maxDepth: number): Promise<boolean> {
  ...
  const ignored = new Set(['node_modules', '.git', '.rapidkit', 'dist', 'build', 'coverage']);
...
function buildManualRepair(input: {
  ...
  return {
    ...
    refreshCommands: ['npx rapidkit doctor project --json', 'npx rapidkit workspace verify --json'],
    reason: input.reason,
