...
export async function markWorkspacePythonEngineInstalled(
  ...
  await updateJsonFile(path.join(workspacePath, '.rapidkit', 'workspace.json'), (manifest) => {
  ...
  await updateJsonFile(path.join(workspacePath, '.rapidkit', 'toolchain.lock'), (toolchain) => {
