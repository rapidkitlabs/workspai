...
import { getWorkspaceRegistryDirectory } from './platform-capabilities.js';
...
async function readWorkspaceManifest(workspacePath: string): Promise<{
  ...
  try {
    ...
    return { exists: true, legacyProjectCount: 0, workspaceName: path.basename(workspacePath) };
  }
}
...
export async function resolveWorkspaceRegisteredProjects(workspacePath: string): Promise<{
  ...
  const contractPath = path.join(resolvedWorkspacePath, WORKSPACE_CONTRACT_PATH);
  ...
  const contractExists = await fsExtra.pathExists(contractPath);
...
export async function readWorkspaceRegistrySummary(
  ...
  if (!(await fsExtra.pathExists(summaryPath))) {
    return null;
  }
  try {
...
export async function publishWorkspaceRegistrySummary(
  ...
  await fsExtra.ensureDir(path.dirname(registrySummaryPath));
  await fsExtra.writeJson(registrySummaryPath, summary, { spaces: 2 });
  return summary;
}
