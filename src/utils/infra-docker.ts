...
export function explainDockerFailure(stderr: string): string | null {
  ...
  if (normalized.includes('address already in use') || normalized.includes('bind')) {
    return 'Docker failed to bind a host port. Stop the conflicting service or adjust ports in contracts/infra-stack.v1.json via .rapidkit/infra/overrides.json.';
  }
