...
const LOG_FORMAT_ENV = 'WORKSPAI_LOG_FORMAT';
const LEGACY_LOG_FORMAT_ENV = 'RAPIDKIT_LOG_FORMAT';

export function resolveCliLogFormat(argv: readonly string[] = process.argv): CliLogFormat {
  const envValue =
    process.env[LOG_FORMAT_ENV]?.trim().toLowerCase() ||
    process.env[LEGACY_LOG_FORMAT_ENV]?.trim().toLowerCase();
  if (envValue === 'json') {
    return 'json';
  }
  if (envValue === 'text') {
