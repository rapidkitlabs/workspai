...
export class JavaRuntimeAdapter implements RuntimeAdapter {
  ...
  private withJavaCacheEnv<T>(projectPath: string, fn: () => Promise<T>): Promise<T> {
    ...
    const mavenCache =
      mode === 'shared-runtime-caches'
        ? path.join(workspace || projectPath, '.workspai', 'cache', 'java', 'm2')
        : path.join(projectPath, '.workspai', 'cache', 'java', 'm2');
    const gradleCache =
      mode === 'shared-runtime-caches'
        ? path.join(workspace || projectPath, '.workspai', 'cache', 'java', 'gradle')
        : path.join(projectPath, '.workspai', 'cache', 'java', 'gradle');
  ...
  async doctorHints(_projectPath: string): Promise<string[]> {
    return [
      ...
      'Run npx workspai init after dependency changes to warm Java caches for your build tool.',
