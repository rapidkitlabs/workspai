...
describe('workspace-paths', () => {
  ...
  it('detects legacy workspace paths under old managed parents', () => {
    const homeDir = '/home/test-user';
    const legacyWorkspace = path.join(getLegacyWorkspacesDirectory(homeDir), 'my-mini-wsp');
    const classicWorkspace = path.join(getClassicWorkspacesDirectory(homeDir), 'my-mini-wsp');
    ...
    expect(isLegacyWorkspacePath(classicWorkspace, homeDir)).toBe(true);
  ...
  it('normalizes legacy workspace artifact paths to canonical Workspai paths', () => {
    expect(toWorkspaiArtifactPath('.rapidkit-workspace')).toBe('.workspai-workspace');
    expect(toWorkspaiArtifactPath('.rapidkit/workspace.json')).toBe('.workspai/workspace.json');
    expect(toWorkspaiArtifactPath('apps/api/.rapidkit/project.json')).toBe(
      'apps/api/.workspai/project.json'
    );
  });

  it('can derive legacy artifact paths for compatibility probes', () => {
    expect(toLegacyRapidkitArtifactPath('.workspai-workspace')).toBe('.rapidkit-workspace');
    expect(toLegacyRapidkitArtifactPath('.workspai/workspace.json')).toBe(
      '.rapidkit/workspace.json'
    );
    expect(toLegacyRapidkitArtifactPath('apps/api/.workspai/project.json')).toBe(
      'apps/api/.rapidkit/project.json'
    );
  });
});
