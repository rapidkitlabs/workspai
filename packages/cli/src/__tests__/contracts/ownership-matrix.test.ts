...
function extractWrapperOwnedCommands(matrixText: string): string[] {
  ...
  const sectionEnd = matrixText.indexOf('### 1.1) Wrapper-owned scoped commands');
...
function extractWrapperOwnedScopedCommands(matrixText: string): string[] {
  const sectionStart = matrixText.indexOf('### 1.1) Wrapper-owned scoped commands');
  const sectionEnd = matrixText.indexOf('### 2) Wrapper-orchestrated project commands');
  const section =
    sectionStart >= 0 && sectionEnd >= 0 ? matrixText.slice(sectionStart, sectionEnd) : '';
...
describe('command ownership matrix drift guard', () => {
  ...
  it('documents all npm wrapper-owned scoped commands', async () => {
    const { NPM_ONLY_SCOPED_COMMANDS } = await import('../../index.js');
    const matrixPath = path.join(process.cwd(), 'docs', 'contracts', 'COMMAND_OWNERSHIP_MATRIX.md');
    const matrixText = fs.readFileSync(matrixPath, 'utf-8');

    const documented = new Set(extractWrapperOwnedScopedCommands(matrixText));
    const expected = new Set<string>(
      NPM_ONLY_SCOPED_COMMANDS.map((command: readonly string[]) => command.join(' '))
    );
