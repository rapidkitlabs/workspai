...
Complete examples showing how to use Workspai AI for different scenarios.
...
$ npx workspai ai recommend "social login with Google Facebook Twitter OAuth"
...
$ npx workspai ai recommend "image upload storage S3 with thumbnails"
...
$ npx workspai ai recommend "real-time push notifications websocket"
...
npx workspai add module authentication-core users-core storage redis-cache
```
...
$ npx workspai ai recommend "user authentication with email password reset"
...
$ npx workspai ai recommend "subscription billing monthly yearly Stripe webhooks"
...
$ npx workspai ai recommend "background jobs async tasks Celery"
...
```bash
npx workspai add module authentication-core users-core stripe-payment email celery redis-cache
```
...
$ npx workspai ai recommend "PostgreSQL database with full-text search"
...
$ npx workspai ai recommend "payment checkout Stripe with order management"
...
$ npx workspai ai recommend "background tasks for inventory sync cron"
...
```bash
npx workspai add module db-postgres redis-cache stripe-payment email celery
```
...
$ npx workspai ai recommend "secure authentication HIPAA compliant audit logging"
...
$ npx workspai ai recommend "PostgreSQL with encryption for sensitive data"
...
$ npx workspai ai recommend "SMS notifications Twilio appointment reminders"
...
```bash
npx workspai add module authentication-core session-management db-postgres storage sms celery redis-cache
```
...
```bash
# Step 1: User System
$ npx workspai ai recommend "user accounts with profiles avatars achievements"
...
$ npx workspai ai recommend "leaderboards rankings Redis sorted sets"
...
$ npx workspai ai recommend "in-app purchases mobile payment Stripe"
...
```bash
npx workspai add module authentication-core users-core redis-cache stripe-payment db-mongodb
```
...
❌ Bad:
```bash
npx workspai ai recommend "social media app"
```

✅ Good:
```bash
npx workspai ai recommend "user authentication social login"
npx workspai ai recommend "image upload storage S3"
...
npx workspai ai recommend "PostgreSQL database with async support"
```
...
```bash
# Vague
npx workspai ai recommend "payments"

# Specific (much better!)
npx workspai ai recommend "subscription payments with monthly billing and invoices"
```
...
npx workspai ai recommend "auth jwt oauth redis"

# Describe what you're building
npx workspai ai recommend "I need authentication with JWT tokens and Redis sessions for my API"
```
...
```bash
# 1. Install modules
npx workspai add module <module-id-1> <module-id-2> ...

# 2. Configure modules
npx workspai modules configure <module-id>
...
**Need help?** Open an issue: https://github.com/rapidkitlabs/workspai/issues
