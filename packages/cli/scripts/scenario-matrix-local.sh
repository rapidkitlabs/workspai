...
# Practical end-to-end scenario matrix for Workspai CLI (npm) WITHOUT Docker.
...
if [[ ! -f "$OUT_A1_DIR/offline-api/.workspai/project.json" && ! -f "$OUT_A1_DIR/offline-api/.rapidkit/project.json" ]]; then
  echo "❌ Expected fallback project to include .workspai/project.json or legacy .rapidkit/project.json" >&2
  exit 1
fi
...
# global-ish
OUT_D1="$(PATH="$GLOBAL_BIN:$NODE_BIN:$SYS_PATH_BASE" workspai --help 2>&1 | head -n 40)"
if [[ -z "$OUT_D1" ]]; then
  echo "❌ Expected workspai --help to print output" >&2
  exit 1
fi
if ! echo "$OUT_D1" | grep -q "Workspai"; then
  echo "❌ Expected Workspai CLI help output (conflict/path issue?). Got:" >&2
  echo "$OUT_D1" >&2
  exit 1
fi
...
OUT_D2="$(PATH="$NODE_BIN:$SYS_PATH_BASE" ./node_modules/.bin/workspai --help 2>&1 | head -n 40)"

# project-local should also forward core commands (and fail gracefully without python)
CACHE_D="$TMP/cache-d"
mkdir -p "$CACHE_D"
set +e
OUT_D3="$(PATH="$NODE_BIN:$SYS_PATH_BASE" XDG_CACHE_HOME="$CACHE_D" ./node_modules/.bin/workspai list --json 2>&1)"
CODE_D3=$?
set -e
popd >/dev/null

if [[ -z "$OUT_D2" ]]; then
  echo "❌ Expected npx workspai --help to print output" >&2
  exit 1
fi
if ! echo "$OUT_D2" | grep -q "Workspai"; then
  echo "❌ Expected project-local Workspai CLI help output. Got:" >&2
  echo "$OUT_D2" >&2
  exit 1
fi
