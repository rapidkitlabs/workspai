...
#
# Inputs:
#   Optional: WORKSPAI_E2E_NPM_TARBALL=/abs/path/to/workspai-cli-*.tgz
...
#
# Optional:
#   RAPIDKIT_E2E_FULL=1   (run `workspai init` inside created project)
...
TS="$(date +%s)"
BASE="/tmp/workspai-cli-e2e-user-$TS"
...
TARBALL="${WORKSPAI_E2E_NPM_TARBALL:-${RAPIDKIT_E2E_NPM_TARBALL:-}}"

if [[ -z "$TARBALL" ]]; then
  if [[ ! -d "$NPM_DIR" ]]; then
    echo "E2E(user): Workspai CLI package directory not found: $NPM_DIR" >&2
    exit 1
  fi
...
if ! command -v workspai >/dev/null 2>&1; then
  echo "E2E(user): workspai CLI not found on PATH after install" >&2
  exit 1
fi

step "workspai --version" workspai --version >/dev/null
step "workspai version --json" workspai version --json > "$BASE/version.json"
...
mkdir -p "$WORKSPACES"
step "create project" workspai create project fastapi.standard e2e-app --output "$WORKSPACES" --skip-essentials --force

PROJECT_DIR="$WORKSPACES/e2e-app"
if [[ ! -f "$PROJECT_DIR/.workspai/project.json" && ! -f "$PROJECT_DIR/.rapidkit/project.json" ]]; then
  echo "E2E(user): expected .workspai/project.json or legacy .rapidkit/project.json in created project" >&2
  exit 1
fi

if [[ "${RAPIDKIT_E2E_FULL:-}" == "1" ]]; then
  step "project init (full)" bash -c "cd '$PROJECT_DIR' && workspai init"
fi

# Community safety: ui must not exist.
set +e
workspai ui serve >/dev/null 2>"$BASE/ui.err"
CODE=$?
set -e
if [[ "$CODE" -ne 2 ]]; then
